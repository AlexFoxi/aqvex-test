...
export const ProductFilters: React.FC<Props> = ({ onFilterChange, initialValues }) => {
  ...
  return (
    <div className={cn('flex w-full max-w-[453px] flex-col gap-5')}>
      {/* Search Input Group */}
      <div className={cn('relative flex-1')}>
        <Search
          className={cn('absolute top-1/2 left-4 -translate-y-1/2 text-[#182A42]')}
          size={20}
        />
        <input
          {...register('search')}
          type="text"
          placeholder="Поиск"
          aria-label="Поиск товарів"
          className={cn(
            'w-full rounded-2xl bg-[#F6F7F9] py-2.5 pr-4 pl-12',
            'text-[16px] text-[#182A42] placeholder-[#182A42] outline-none',
      ...
      <div className={cn('relative flex w-full max-w-62 min-w-46.5 items-center self-end')}>
        ...
        <select
          ...
          className={cn(
            ...
            'cursor-pointer text-[16px] font-medium text-[#182A42] outline-none dark:text-white',
          )}
        >
